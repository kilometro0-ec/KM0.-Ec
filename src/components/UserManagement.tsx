...
export default function UserManagement() {
  const { role, user: currentUser, registeredUsers, register, deleteUser, updateAnyUser } = useAuth();
  ...
  const handleDownloadCredentials = (user: any) => {
    const content = `KILOMETRO 0 - CREDENCIALES DE ACCESO\n\nNombre: ${user.name}\nEmail: ${user.email}\nContraseña: ${user.password || '********'}\nRol: ${user.role}\n\nEste documento es confidencial.`;
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `credenciales_${user.name.replace(/\s+/g, '_')}.txt`;
    a.click();
    notify('Credenciales Descargadas', 'El archivo TXT se ha generado satisfactoriamente.', 'success');
  };

  const handleSuspend = (user: any) => {
    const nextStatus = user.status === 'suspended' ? 'active' : 'suspended';
    updateAnyUser(user.email, { status: nextStatus });
    setSelectedUser({ ...user, status: nextStatus });
    notify(
      nextStatus === 'suspended' ? 'Usuario Suspendido' : 'Usuario Reactivado',
      `El acceso para ${user.name} ha sido ${nextStatus === 'suspended' ? 'bloqueado temporalmente' : 'restaurado'}.`,
      nextStatus === 'suspended' ? 'warning' : 'success'
    );
  };

  const handleDelete = (email: string) => {
    if (confirm('¿Estás seguro de eliminar este usuario DEFINITIVAMENTE? Esta acción no se puede deshacer.')) {
      deleteUser(email);
      setSelectedUser(null);
      notify('Usuario Eliminado', 'La cuenta ha sido borrada del sistema central.', 'alert' as any);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end border-b-2 border-km0-orange pb-6">
        <div>
          <h1 className="text-4xl font-black italic uppercase tracking-tighter text-km0-black">Gestión de <span className="text-km0-orange">Sistemas</span></h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-4 bg-white border border-gray-100 rounded-2xl focus:outline-none focus:border-km0-orange transition-all outline-none text-sm font-bold"
          />
        </div>
        <div className="flex gap-2">
          <select 
            ...
            className="px-6 py-4 bg-white border border-gray-100 rounded-2xl text-xs font-black uppercase tracking-widest focus:border-km0-orange outline-none transition-all"
          >
        ...
        <AnimatePresence mode="popLayout">
          {filteredUsers.map((user) => (
            <motion.div
              ...
              className="card-utility p-6 bg-white border border-gray-100 relative group overflow-hidden border-b-4 border-b-black hover:border-km0-orange transition-all"
            >
              <div className="flex items-start justify-between mb-6">
                <div className={`p-4 rounded-2xl shadow-lg transition-transform group-hover:scale-110 ${
                  ...
                  user.role === 'tienda' ? 'bg-km0-orange text-white' : 'bg-gray-100 text-gray-400'
                }`}>
              ...
              <div className="space-y-1">
                <h3 className="font-black italic uppercase tracking-tighter text-xl text-km0-black">{user.name}</h3>
                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  <Mail className="w-3 h-3 text-km0-orange" />
                  {user.email}
                </div>
                {user.parentId && (
                  <div className="mt-2 text-[9px] font-black text-white bg-km0-orange px-2 py-0.5 rounded inline-block uppercase tracking-wider">
                    DE: {user.parentId}
                  </div>
                )}
              </div>
            ...
            >
              <div className="p-8">
                <div className="flex justify-between items-center mb-8">
                  <div>
                    <h3 className="text-2xl font-black italic uppercase tracking-tighter text-km0-black">Nuevo <span className="text-km0-orange">Registro</span></h3>
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm focus:border-km0-orange outline-none transition-all font-bold"
                      />
                    </div>
                    <div className="relative">
                       ...
                       {[
                         ...
                         <button
                          ...
                          className={`p-4 rounded-2xl border text-[10px] font-black uppercase tracking-widest transition-all text-center ${
                            formData.role === r.id
                              ? 'bg-km0-orange border-km0-orange text-white shadow-lg shadow-orange-500/20'
              ...
              <div className="p-8">
                <div className="flex justify-between items-start mb-8">
                  <div className={`p-4 rounded-3xl ${
                    selectedUser.role === 'tienda' ? 'bg-km0-orange' : 'bg-black'
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <h3 className="text-3xl font-black italic uppercase tracking-tighter text-km0-black leading-none">{selectedUser.name}</h3>
                    <p className="text-gray-400 font-bold mt-2 flex items-center gap-2">
                       <Mail className="w-4 h-4 text-km0-orange" />
                       {selectedUser.email}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                      ...
                      <p className="text-xs font-black uppercase text-km0-black tracking-wider">{selectedUser.role}</p>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                      <p className="text-[8px] font-black text-gray-300 uppercase tracking-widest mb-1">Estado Cuenta</p>
                      <p className={`text-xs font-black uppercase tracking-wider ${
                        selectedUser.status === 'suspended' ? 'text-red-500' : 'text-green-500'
                      }`}>
                        {selectedUser.status === 'suspended' ? 'Suspendida' : 'Activa'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <button 
                      onClick={() => handleDownloadCredentials(selectedUser)}
                      className="w-full py-4 bg-black text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-km0-orange transition-all shadow-xl shadow-black/10"
                    >
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-gray-50 text-gray-500 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-gray-100 transition-all"
                    >
                      {selectedUser.status === 'suspended' ? 'Reactivar Cuenta' : 'Suspender Temporalmente'}
                    </button>
                  </div>
                </div>
              </div>
